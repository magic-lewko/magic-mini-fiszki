// Buduje dist/: pliki z zrodlo/, algorytm lib/fsrs.mjs i wygenerowany service worker z pelna lista plikow.
// WERSJA to poczatek sha256 z zawartosci wszystkich plikow, wiec kazda zmiana daje nowy cache i baner aktualizacji.
// Slowek nie ma w dist/: uzytkownik dodaje je w apce (wklejenie albo plik), trafiaja do IndexedDB.
// Uruchomienie: node zbuduj.mjs

import { createHash } from 'node:crypto'
import { copyFileSync, existsSync, mkdirSync, readFileSync, readdirSync, rmSync, statSync, writeFileSync } from 'node:fs'
import { dirname, join } from 'node:path'
import { fileURLToPath } from 'node:url'

const KORZEN = dirname(fileURLToPath(import.meta.url))
const ZRODLO = join(KORZEN, 'zrodlo')
const DIST = join(KORZEN, 'dist')
const FSRS = join(KORZEN, 'lib', 'fsrs.mjs')
const WYMAGANE = ['index.html', 'app.js', 'styl.css', 'manifest.webmanifest', 'ikona-180.png', 'ikona-192.png', 'ikona-512.png']

// Service worker. W Node ta funkcja nie jest wywolywana: jej tekst trafia do dist/sw.js za stalymi WERSJA i PLIKI.
function pracownik() {
  const CACHE = 'mmf-' + WERSJA

  // Atomowo: najpierw pobierane sa wszystkie pliki i dopiero gdy kazdy ma status ok, trafiaja do cache. Brak
  // ktoregokolwiek przerywa instalacje, a stara wersja dziala dalej. Parametr v omija cache CDN (GitHub Pages stoi
  // za Fastly, ktory mogl oddac stary plik pod nowa wersja), cache: 'reload' omija cache HTTP. Klucz w cache jest
  // bez parametru, wiec dopasowanie w fetch i w statusie jest dokladne.
  // Bez skipWaiting: nowa wersja czeka, az uzytkownik sam kliknie baner.
  async function zainstaluj() {
    const odpowiedzi = await Promise.all(
      PLIKI.map(async (plik) => {
        const adres = plik + (plik.includes('?') ? '&' : '?') + 'v=' + WERSJA
        const odpowiedz = await fetch(new Request(adres, { cache: 'reload' }))
        if (!odpowiedz.ok) throw new Error(`${plik}: HTTP ${odpowiedz.status}`)
        return odpowiedz
      }),
    )
    const cache = await caches.open(CACHE)
    try {
      await Promise.all(PLIKI.map((plik, i) => cache.put(new Request(plik), odpowiedzi[i])))
    } catch (blad) {
      // Niepelny cache tej wersji nie moze zostac: nazwa jest unikalna dla wersji, wiec usuwamy tylko jego.
      await caches.delete(CACHE)
      throw blad
    }
  }

  self.addEventListener('install', (zdarzenie) => {
    zdarzenie.waitUntil(zainstaluj())
  })

  self.addEventListener('activate', (zdarzenie) => {
    zdarzenie.waitUntil(
      caches
        .keys()
        .then((nazwy) => Promise.all(nazwy.filter((n) => n.startsWith('mmf-') && n !== CACHE).map((n) => caches.delete(n))))
        .then(() => self.clients.claim()),
    )
  })

  const zCache = (zadanie) => caches.open(CACHE).then((cache) => cache.match(zadanie, { ignoreSearch: true }))

  // Safari odrzuca nawigacje obsluzona odpowiedzia z flaga przekierowania (hosting moze zamienic index.html na /).
  async function bezPrzekierowania(odpowiedz) {
    if (!odpowiedz.redirected) return odpowiedz
    const tresc = await odpowiedz.blob()
    return new Response(tresc, { status: odpowiedz.status, statusText: odpowiedz.statusText, headers: odpowiedz.headers })
  }

  // Zawsze najpierw cache, siec tylko gdy pliku w nim nie ma. Dzieki temu odswiezenie bez sieci dziala.
  self.addEventListener('fetch', (zdarzenie) => {
    const zadanie = zdarzenie.request
    if (zadanie.method !== 'GET') return
    if (new URL(zadanie.url).origin !== self.location.origin) return
    if (zadanie.mode === 'navigate') {
      zdarzenie.respondWith(zCache('./index.html').then((odp) => (odp ? bezPrzekierowania(odp) : fetch(zadanie))))
      return
    }
    zdarzenie.respondWith(zCache(zadanie).then((odp) => odp || fetch(zadanie)))
  })

  self.addEventListener('message', (zdarzenie) => {
    if (zdarzenie.data === 'status') {
      const port = zdarzenie.ports[0]
      zdarzenie.waitUntil(
        caches
          .open(CACHE)
          .then((cache) => Promise.all(PLIKI.map((plik) => cache.match(plik, { ignoreSearch: true }))))
          .then((wyniki) => port?.postMessage({ wersja: WERSJA, pliki: PLIKI.length, zapisane: wyniki.filter(Boolean).length })),
      )
    } else if (zdarzenie.data === 'aktualizuj') {
      self.skipWaiting()
    }
  })
}

function zbuduj() {
  for (const plik of WYMAGANE) {
    if (!existsSync(join(ZRODLO, plik))) {
      const podpowiedz = plik.endsWith('.png') ? ' Uruchom: node narzedzia/ikony.mjs' : ''
      throw new Error(`Brak zrodlo/${plik}.${podpowiedz}`)
    }
  }
  const nazwy = readdirSync(ZRODLO)
    .filter((n) => statSync(join(ZRODLO, n)).isFile())
    .sort()
  for (const zajeta of ['sw.js', 'fsrs.mjs']) {
    if (nazwy.includes(zajeta)) throw new Error(`zrodlo/${zajeta} koliduje z plikiem generowanym przy budowaniu`)
  }

  rmSync(DIST, { recursive: true, force: true })
  mkdirSync(DIST, { recursive: true })
  for (const n of nazwy) copyFileSync(join(ZRODLO, n), join(DIST, n))
  copyFileSync(FSRS, join(DIST, 'fsrs.mjs'))
  // GitHub Pages bez tego pliku przepuszcza strone przez Jekylla.
  writeFileSync(join(DIST, '.nojekyll'), '')

  const wszystkie = [...nazwy, 'fsrs.mjs'].sort()
  sprawdzOdwolania(wszystkie)

  const skrot = createHash('sha256')
  // Tekst service workera tez wchodzi do wersji: zmiana samego szablonu z ta sama nazwa cache instalowalaby sie
  // w cache aktywnej wersji.
  skrot.update(pracownik.toString())
  for (const n of wszystkie) {
    skrot.update(n + '\0')
    skrot.update(readFileSync(join(DIST, n)))
  }
  const wersja = skrot.digest('hex').slice(0, 12)
  const pliki = ['./', './index.html', ...wszystkie.filter((n) => n !== 'index.html').map((n) => './' + n)]

  const sw = [
    '// Wygenerowane przez zbuduj.mjs. Nie edytuj recznie.',
    `const WERSJA = '${wersja}'`,
    `const PLIKI = ${JSON.stringify(pliki, null, 2).replaceAll('"', "'")}`,
    '',
    `;(${pracownik.toString()})()`,
    '',
  ].join('\n')
  writeFileSync(join(DIST, 'sw.js'), sw)

  const bajty = wszystkie.reduce((suma, n) => suma + statSync(join(DIST, n)).size, 0)
  console.log(`dist/ gotowe: wersja ${wersja}, ${pliki.length} wpisow w PLIKI, ${(bajty / 1024).toFixed(1)} KB`)
  for (const p of pliki) console.log(`  ${p}`)
}

// Plik, do ktorego odwoluje sie apka, a ktorego nie ma w dist/, zepsulby tryb offline dopiero na telefonie.
function sprawdzOdwolania(nazwy) {
  const brakujace = []
  const sprawdz = (zrodlo, cel) => {
    const czysty = cel.replace(/^\.\//, '').split(/[?#]/)[0]
    if (czysty && !nazwy.includes(czysty)) brakujace.push(`${zrodlo} -> ${cel}`)
  }
  for (const n of nazwy.filter((x) => /\.(m?js)$/.test(x))) {
    for (const [, cel] of readFileSync(join(DIST, n), 'utf8').matchAll(/(?:from|import)\s*\(?\s*'(\.\/[^']+)'/g)) sprawdz(n, cel)
  }
  for (const [, cel] of readFileSync(join(DIST, 'index.html'), 'utf8').matchAll(/(?:href|src)="(\.\/[^"]*)"/g)) sprawdz('index.html', cel)
  for (const ikona of JSON.parse(readFileSync(join(DIST, 'manifest.webmanifest'), 'utf8')).icons || []) {
    sprawdz('manifest.webmanifest', ikona.src)
  }
  if (brakujace.length) throw new Error(`Odwolania do plikow, ktorych nie ma w dist/:\n${brakujace.join('\n')}`)
}

try {
  zbuduj()
} catch (blad) {
  console.error(`Budowanie nie powiodlo sie: ${blad.message}`)
  process.exit(1)
}
